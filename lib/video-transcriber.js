...
const TEMP_AUDIO_DIR = envVar('TEMP_AUDIO') || path.join(ROOT, 'temp_audio');
...
function isFasterWhisperAvailable() {
  return checkTranscriber().ok;
}

/**
 * Why can't we transcribe? — enough detail for the UI to say something useful.
 *
 * Two failure modes need different instructions: no Python at all, versus a
 * Python without the faster-whisper package. Reporting them apart is the whole
 * point; "subtitles unavailable" sends people looking in the wrong place.
 *
 * Honours PYTHON_PATH like the diarizer and translator already do — this check
 * used to hardcode `python`, so a user who pointed Vault at a venv still got
 * told the package was missing.
 *
 * Only a POSITIVE result is cached. A cached negative outlives its truth: pip
 * install faster-whisper into an already-visible Python and the very next check
 * should see it, without restarting Vault. (A newly-installed *interpreter* is
 * different — this process keeps the PATH it started with — which is why the UI
 * says to restart in that case.)
 */
function checkTranscriber() {
  if (_fasterWhisperAvailable) return _fasterWhisperAvailable;

  const pythonCmd = process.env.PYTHON_PATH || 'python';
  const run = (args) => {
    try {
      return spawnSync(pythonCmd, args, { encoding: 'utf-8', windowsHide: true, timeout: 10000 });
    } catch {
      return null;
    }
  };
  ...
  const python = !!ver && ver.error == null && ver.status === 0;
  if (!python) return { ok: false, python: false, fasterWhisper: false, pythonCmd };

  const show = run(['-m', 'pip', 'show', 'faster-whisper']);
  const fasterWhisper = !!show?.stdout?.includes('faster-whisper');
  const result = { ok: fasterWhisper, python: true, fasterWhisper, pythonCmd };
  if (result.ok) _fasterWhisperAvailable = result;   // cache the good news only
  return result;
}
...
async function startServerInner({ onDownloading } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Consent is per model: approval for large-v3-turbo isn't approval to fetch
    // a different one later if WHISPER_MODEL changes. The sidecar tries the
    // on-disk cache first regardless, so a '0' here only forbids the network.
    const mayDownload = require('./model-consent').request(`whisper:${model}`, { model });

    whisperServer = spawn(process.env.PYTHON_PATH || 'python', ['-X', 'utf8', '-u', scriptPath, model, device, compute], {
      ...
      env: {
        ...
        SUB_ALLOW_DOWNLOADS: mayDownload ? '1' : '0',
...
async function transcribeVideo(videoPath, options = {}) {
  ...
  try {
    ...
    // A dead sidecar took the loaded Whisper model with it — every later video
    // would silently come back untranscribed. Surface it so the scan queue
    // halts and the user can restart the model instead of losing the run.
    // (The 'close' handler nulls whisperServer, so a Resume respawns it.)
