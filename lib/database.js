...
function init(dbPath = config.paths.database, password = config.getDbPassword()) {
  ...
  db.exec(`
    ...
    -- HLS remux streaming (see lib/stream/). Both tables live in the MAIN
    -- database, so in vault mode they are encrypted like everything else and
    ...
    -- What the segment store holds per file, for eviction and for "is this
    -- file already fully cached".
    CREATE TABLE IF NOT EXISTS stream_cache (
      media_id      INTEGER PRIMARY KEY,
      bytes         INTEGER NOT NULL DEFAULT 0,
      segment_count INTEGER NOT NULL DEFAULT 0,   -- total in the playlist
      have_mask     TEXT NOT NULL DEFAULT '',     -- hex bitset, bit n = segment n present
      complete      INTEGER NOT NULL DEFAULT 0,
      last_access   INTEGER NOT NULL,
      created_at    INTEGER NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_stream_cache_access ON stream_cache(last_access);
  `);
...
/**
 * Does this id still name a live (not trashed) media row? Cheap enough to call
 * on a hot path — used by the segment store to refuse writes for a record that
 * has been deleted out from under a producer.
 */
function mediaLives(id) {
  const row = get().prepare(
    'SELECT user_trashed FROM media WHERE id = ?'
  ).get(id);
  return !!row && !row.user_trashed;
}
...
function deleteRecords(ids) {
  ...
  // Stop every remux producer BEFORE the transaction opens. A live producer
  // keeps re-putting segments, which recreates the cache directory and the
  // stream_cache row the deletes below are about to remove. killNow() is
  // synchronous on purpose: this transaction cannot await anything.
  for (const id of ids) {
  ...
  const run = db.transaction((idList) => {
    ...
    for (const id of idList) {
      ...
      // Cached HLS segments follow the record out (both backends), and so does
      // the keyframe index: spec section 2 says BOTH stream tables are cleared
      // when a media row goes, and a stale stream_index row would otherwise
      // outlive every other trace of the file. (service.forget() is the same
      // work plus a session stop, but it is async and this runs inside a
      // transaction, so the two table deletes are done directly here, with the
      // session stopped by the killNow() sweep above.)
      try { require('./stream/store').deleteAll(id); } catch {}
...
// ── Streaming: codec columns, keyframe index, segment-cache bookkeeping ─────
...
function getStreamCache(mediaId) {
  return get().prepare('SELECT * FROM stream_cache WHERE media_id = ?').get(mediaId) || null;
}

/** Upsert the cache row. `patch` may carry bytes/segment_count/have_mask/complete. */
function saveStreamCache(mediaId, patch) {
  const db = get();
  const now = Date.now();
  db.prepare(`
    INSERT INTO stream_cache (media_id, bytes, segment_count, have_mask, complete, last_access, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(media_id) DO UPDATE SET
      bytes = excluded.bytes, segment_count = excluded.segment_count,
      have_mask = excluded.have_mask, complete = excluded.complete,
      last_access = excluded.last_access
  `).run(mediaId,
    // `| 0` would wrap a file bigger than 2 GB round to a negative total.
    Math.max(0, Math.round(Number(patch.bytes) || 0)),
    patch.segment_count | 0, patch.have_mask || '',
    patch.complete ? 1 : 0, now, now);
}

function touchStreamCache(mediaId) {
  get().prepare('UPDATE stream_cache SET last_access = ? WHERE media_id = ?').run(Date.now(), mediaId);
}

function deleteStreamCacheRow(mediaId) {
  return get().prepare('DELETE FROM stream_cache WHERE media_id = ?').run(mediaId).changes;
}

/** { bytes, files } across the whole segment cache. */
function streamCacheTotals() {
  const r = get().prepare('SELECT COALESCE(SUM(bytes),0) AS bytes, COUNT(*) AS files FROM stream_cache').get();
  return { bytes: r.bytes || 0, files: r.files || 0 };
}

/** Cache rows oldest-first — the eviction order. */
function streamCacheByAge() {
  return get().prepare('SELECT media_id, bytes FROM stream_cache ORDER BY last_access ASC').all();
}

function allStreamCacheIds() {
  return get().prepare('SELECT media_id FROM stream_cache').all().map(r => r.media_id);
}

function clearStreamCacheRows() {
  return get().prepare('DELETE FROM stream_cache').run().changes;
}
...
/**
 * Store the generated thumbnail path for a media row
 */
function setThumbnailPath(id, thumbPath) {
  const db = get();
  db.prepare('UPDATE media SET thumbnail_path = ? WHERE id = ?').run(thumbPath, id);
}
...
module.exports = {
  ...
  saveStreamCache,
  touchStreamCache,
  deleteStreamCacheRow,
  streamCacheTotals,
  streamCacheByAge,
  allStreamCacheIds,
  clearStreamCacheRows,
  init,
  get,
  close,
  verifyPassword,
