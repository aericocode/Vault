...
function isModelUnavailable(err) {
  ...
  if (status === 400 && MODEL_GONE_PHRASES.some(p => body.includes(p))) {
    // Verified live against LM Studio 0.3.x: eject a copy while it is answering
    // and the open request comes back 400 `{"error":"Model unloaded by user or
    // API request."}`. A 400 normally means "your request was bad", which is
    // why only the "which model?" case used to be read out of one — but a 400
    // that names the MODEL as the problem is the JIT-unload shape wearing a
    // different status. Reading it as a bad file is how ejecting one copy of
    // three mid-scan marked the two files it was answering for as failed
    // instead of retrying them on the copies that were still up.
    return true;
  }
...
function describe(err) {
  ...
  const status = Number(err.status);
  if (status === 400 && /model unloaded|not loaded/i.test(String(err.body || msg))) {
    return 'That copy of the model was unloaded while it was answering';
  }
