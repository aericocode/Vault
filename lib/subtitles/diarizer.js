...
async function diarize(wavPath, { onProgress, onPct, numSpeakers, threshold, timeoutMs = 20 * 60 * 1000 } = {}) {
  if (!isProvisioned()) {
    ...
    // Its own consent key — nothing else the user approved implies this one.
    // Upstream catches this and keeps plain (unlabelled) cues.
    if (!require('../model-consent').request('diarize')) {
      throw _err('DOWNLOADS_OFF',
        `diarization models aren't installed and haven't been approved for download. ` +
        `Approve it when Vault offers, pre-place ${SEG_NAME} and ${EMB_NAME} in ${MODEL_DIR}, ` +
        `or set SUB_ALLOW_DOWNLOADS=1.`);
    }
