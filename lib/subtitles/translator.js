    ...
    if (blocked) {
      ...
      try {
        if (!isProvisioned(srcLang)) {
          ...
          // Consent is per language pair: approving the Whisper model months
          // ago is not approval to fetch a Japanese pack today. Unapproved
          // simply means we don't reach out — translation still happens, via
          // the local LM Studio fallback below — and the pair is recorded so
          // the UI can offer the choice.
          if (!require('../model-consent').request(`opus:${srcLang}`, { pair: `${srcLang}-en` })) {
            throw _err('DOWNLOADS_OFF',
              `OPUS-MT ${srcLang}-en isn't installed and hasn't been approved for download. Using the LLM instead`);
          }
