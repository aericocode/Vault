...
async function _healFfprobeRows() {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // Same probe, so the codec columns come along for free.
      try { db.saveStreamInfo(row.id, info); } catch {}
      healed++;
...
async function run({ reason = 'startup' } = {}) {
  ...
  if (out.ffprobe) {
    console.log(`[Self-heal] re-probed ${out.ffprobe} file(s) after ffmpeg became available (${reason}) — they now show as ⏳ unscanned; the next scan or a per-file rescan picks them up`);
  }
  if (out.whisper) {
    console.log(`[Self-heal] reset ${out.whisper} audio file(s) now that faster-whisper is available (${reason}) — they now show as ⏳ unscanned; the next scan or a per-file rescan picks them up`);
  }
  return out;
}
