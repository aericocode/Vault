...
const VALID_PURPOSES = ['model', 'update', 'llm'];
...
function gate(url, purpose) {
  ...
  // 'update' and 'llm' are only gated by VAULT_OFFLINE (handled above).
}
...
async function netFetch(url, opts = {}) {
  ...
  if (!VALID_PURPOSES.includes(purpose)) {
    ...
    throw new Error('netFetch: opts.purpose is required and must be one of model | update | llm');
  }
