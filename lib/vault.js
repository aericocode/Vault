...
const vault = {
  ...
  setPassword(pass) {
    ...
    try {
      require('./stream/session').stopAll();
      require('./stream/store').purgePlaintext();
  ...
  changePassword(current, next) {
    ...
    if (!database.verifyPassword(current)) {
      ...
      if (this._changeFailures >= this.CHANGE_FAIL_LIMIT) {
        ...
        this.lock({ force: true });        // seal it — every queue aborts on 'locking'
  ...
  lock({ force = false } = {}) {
    ...
    try { require('./stream/session').stopAll(); } catch {}
    database.close();
  ...
  startAutolock() {
    ...
    this._timer = setInterval(() => this._autolockTick(), 60 * 1000);
