 ...
 *     -readrate 4 -readrate_initial_burst 60     # see (7)
 ...
 *     rather than hanging.
 *
 * (7) -readrate 4 -readrate_initial_burst 60 (both accepted by this build,
 ...
 *     after that the input is read at 4x realtime. 4 rather than 1.5 because
 *     the player offers 2x and faster playback speeds, and a 1.5x producer
 *     runs out of cushion the moment the burst is spent. Both are INPUT
 *     options and must sit before -i. If a build ever rejects
 *     -readrate_initial_burst, drop that flag alone and lower ring.BEHIND:
 *     -readrate 4 by itself still works, it just makes the first seconds
 *     tighter.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
/**
 * A just-restarted producer is left alone for this long, or until it has
 * delivered the segment it was restarted for, whichever comes first.
 *
 * Two clients on one file otherwise undo each other: client B seeks, the
 * producer restarts at B's segment, and client A's very next request restarts
 * it again before B's segment was ever made, so B waits out the whole WAIT_MS
 * for something nobody is producing any more. During the grace the other
 * client's request waits instead of restarting; if the grace runs out and its
 * segment still is not there, it restarts then, well inside WAIT_MS.
 */
const GRACE_MS = 3000;

/**
 * Read the input at this multiple of realtime once the burst is spent. 4, not
 * 1.5: the player offers 2x and faster playback, and a 1.5x producer stalls a
 * 2x play as soon as the initial burst is used up.
 */
const READRATE = '4';
...
class Session {
  constructor(mediaId, row, idx, plan) {
    ...
    this.target = -1;                    // the segment this run was started for
    this.graceUntil = 0;                 // until when other clients wait instead of restarting
    this.child = null;
    this.dir = null;
    this.dead = false;
    this.error = null;
  ...
  async start(n) {
    ...
    // A CLI scan removes the whole temp folder rather than its contents
    // (lib/frame-extractor.js), and nothing recreates it until the next
    // thumbnail render, so every remux would 503 until then. Make sure the
    // parent exists before carving out our own directory in it.
  ...
  /**
   * Is this run still owed the segment it was started for, recently enough that
   * another client should wait for it rather than restart FFmpeg somewhere else?
   */
  inGrace() {
    return !this.dead && this.head < this.target && Date.now() < this.graceUntil;
  }
 ...
 *
 * @returns {Promise<{session:Session, deferred:boolean, retryInMs:number}>}
 *   `deferred` means no restart happened because the running producer is inside
 *   its grace period; the caller should wait `retryInMs` and ask again.
 */
async function ensure(mediaId, row, idx, plan, n) {
  return serialize(async () => {
    ...
    if (live && !live.dead) {
      ...
      if (n > live.head && n <= live.head + LOOKAHEAD) {
        return { session: live, deferred: false, retryInMs: 0 };
      }
      // ...except while the run is still owed the segment it was started for.
      // Restarting now would strand whoever is waiting on that one.
      if (live.inGrace()) {
        return { session: live, deferred: true, retryInMs: Math.max(50, live.graceUntil - Date.now()) };
      }
      await live.stop();
    }
    ...
    return { session: s, deferred: false, retryInMs: 0 };
  });
}
 ...
 *
 * `retryable` says whether asking again is worth anything: a run that simply
 * ended (or was restarted somewhere else by another client) can be started
 * again, while a run that recorded an FFmpeg error will fail the same way.
 */
    ...
    const tick = () => {
      ...
      if (!s || s.dead) {
        ...
        return resolve({
          ok: false,
          retryable: !err,
          error: err || 'the remux ended before this segment',
        });
      }
      if (Date.now() > deadline) {
        return resolve({ ok: false, retryable: true, error: 'timed out waiting for the remux' });
      }
      setTimeout(tick, 100);
    };
    tick();
...
function status() {
  ...
  for (const s of sessions.values()) {
    ...
    active.push({
      ...
      playheads: st ? st.playheads : [],     // one per client still asking
      clients: st ? st.clients : 0,
      waiting: st ? st.waiting : 0,          // segments a request is blocked on
  ...
  for (const st of byId.values()) {
    active.push({
      ...
      playhead: st.playhead, playheads: st.playheads, clients: st.clients,
      waiting: st.waiting, produced: st.produced, idleMs: st.idleMs,
    });
  }
  return {
    active,
...
module.exports = {
  ...
  IDLE_MS, READRATE, READRATE_BURST, GRACE_MS, LOOKAHEAD,
