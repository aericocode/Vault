 ...
 * all the producer simply keeps going: every segment it finishes after the
 * delete is re-put, which recreates both the stream-cache/<id> directory and the
 * stream_cache row the transaction had just removed (measured: a 64 MB orphan
 * for a deleted id).
 *
 * Marking the session dead is what actually makes the delete stick — `_ingest`
 * refuses on `dead`, so every in-flight completion, and the whole leftover sweep
 * the child's 'close' handler runs, is dropped. Killing the child and sweeping
...
module.exports = {
  ensure, waitFor, stop, killNow, stopAll, activeIds, status,
