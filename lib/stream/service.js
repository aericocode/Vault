...
async function segmentFor(inputRow, n) {
  ...
  // an index. Noting the request is what moves the playhead, so it happens
  // before anything can fail.
  if (Number.isInteger(n) && n >= 0) {
    ring.note(inputRow.id, n);
  ...
  // Only reachable if the ring dropped it between the wait and this read, which
  // takes another session's segments crossing the global cap in that window.
  if (!seg) return { ok: false, status: 503, error: 'the segment was dropped before it could be sent' };
