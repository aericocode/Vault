...
function put(mediaId, n, buf, segmentCount) {
  ...
  const mask = maskSet(row ? row.have_mask : '', n, true);
  const bytes = (row ? row.bytes : 0) + buf.length;
  const total = segmentCount || (row ? row.segment_count : 0);
  db.saveStreamCache(mediaId, {
    bytes,
    segment_count: total,
    have_mask: mask,
    complete: total > 0 && maskCount(mask, total) >= total,
  });
  return bytes;
}
...
function deleteAll(mediaId) {
  ...
  const d = dirFor(mediaId);
  if (fs.existsSync(d) && ownedDir.guardSweep(root(), 'stream-cache delete')) {
...
function purgePlaintext() {
  ...
  if (!ownedDir.guardSweep(dir, 'stream-cache purge')) return 0;
  let n = 0;
  for (const name of fs.readdirSync(dir)) {
    if (name === ownedDir.MARKER_NAME) continue;
 ...
 * Files with a live producer session are never evicted — the player is reading
 * them right now.
...
function evictToCap(protectedIds = new Set()) {
  ...
  const dropped = [];
  for (const row of db.streamCacheByAge()) {
    if (bytes <= cap) break;
    if (protectedIds.has(row.media_id)) continue;
    deleteAll(row.media_id);
    bytes -= row.bytes;
...
module.exports = {
  ...
  has, get, put, touch, deleteAll, clearAll, purgePlaintext,
  capBytes, evictToCap, stats,
