...
function put(mediaId, n, buf, segmentCount) {
  // Belt and braces for the delete paths. A producer is killed before its
  // media row goes, so this should never fire — but a segment that lands after
  // the row is gone would recreate both the cache directory and the
  // stream_cache row with nothing left to ever clean them up again.
  if (!db.mediaLives(mediaId)) return false;
