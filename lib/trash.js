...
async function trashItems(ids) {
  ...
  for (const id of ids) {
    ...
    try {
      // BEFORE the move, not after. The cached HLS segments go with the file: it
      // has no business leaving a playable copy of itself behind in the cache.
      // Doing it afterwards left the producer running across the move, so every
      // segment it finished in the meantime was written back into the cache the
      // delete had just emptied — and on Windows the move itself failed with
      // EBUSY, because the FFmpeg child still had the source file open.
      // forget() is awaited, so the child is really gone by the time we rename.
