 ...
 */
/**
 * Song references in matcher form: [{ id, song_id, fingerprint: Int32Array }].
 * One call reads the whole song_fingerprints table and base64-decodes every
 * row it keeps, so a sweep over many files (seed-pack rescan, CLI import)
 * loads them ONCE and passes the result as `references` to every scan,
 * instead of paying that read + decode again for each file.
 * @param onlyRefIds Set or array of song_fingerprints ids to keep (null = all)
 */
function loadReferences({ onlyRefIds = null } = {}) {
  let refsRaw = repo.listSongFingerprints();
  if (onlyRefIds) {
    const only = onlyRefIds instanceof Set ? onlyRefIds : new Set(onlyRefIds);
    refsRaw = refsRaw.filter(r => only.has(r.id));
  }
  return refsRaw.map(r => ({
    id: r.id, song_id: r.song_id,
    fingerprint: fpx.decodeFingerprint(r.fingerprint),
  }));
}

function matchChunksForMedia(media_id, {
  threshold = SCAN_THRESHOLD, onlyRefIds = null, references = null,
} = {}) {
  const fps = repo.getMediaFingerprints(media_id);
  if (!fps.length) return null;

  // An explicit `references` array is authoritative, even when empty (that
  // means "nothing to match against"); otherwise load and filter them here.
  const refs = Array.isArray(references) ? references : loadReferences({ onlyRefIds });
  if (!refs.length) {
  ...
  return fps.map(f => ({
    ...
    match: matcher.matchChunkAgainstReferences(fpx.decodeFingerprint(f.fingerprint), refs, threshold),
  }));
}

function scanAgainstReferences(media_id, {
  threshold = SCAN_THRESHOLD, onlyRefIds = null, references = null,
} = {}) {
  const chunkResults = matchChunksForMedia(media_id, { threshold, onlyRefIds, references });
...
module.exports = {
  ...
  loadReferences, matchChunksForMedia, getSections, tagSection,
