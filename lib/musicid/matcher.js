 ...
 *
 * Early abandon: an offset is only interesting if it beats the best BER so
 * far (or the caller's `maxBer` ceiling), and error bits only ever
 * accumulate, so once an offset's count exceeds what that ceiling allows
 * over its overlap the rest of the ints are skipped. Almost every offset
 * of a non-matching pair dies within a few ints, which is where nearly all
 * of the matcher's time went. One bit of slack keeps the result identical
 * to a full count: anything abandoned is strictly worse than the best, and
 * anything within a bit of it is counted to the end and judged exactly.
 *
 * `maxBer` contract: when the true best BER is <= maxBer the result is the
 * exact full-count answer; otherwise only `ber > maxBer` is guaranteed and
 * offset/length are meaningless. Callers that pass maxBer must therefore
 * discard any result above it (matchChunkAgainstReferences does).
 */
function bestMatch(a, b, opts = {}) {
  const { maxOffset = null, minOverlap = 8, maxBer = null } = opts;
  ...
  let ceiling = maxBer != null ? maxBer : 1;
  for (let off = lo; off <= hi; off++) {
    let aStart, bStart;
    if (off >= 0) { aStart = off; bStart = 0; }
    else { aStart = 0; bStart = -off; }
    const len = Math.min(aLen - aStart, bLen - bStart);
    if (len <= 0 || len < minOverlap) continue;
    const budget = ceiling * len * 32 + 1;
    let bits = 0, i = 0;
    for (; i < len; i++) {
      bits += popcount32(a[aStart + i] ^ b[bStart + i]);
      if (bits > budget) break;
    }
    if (i < len) continue;                   // abandoned: cannot beat best
    const ber = bits / (len * 32);
    if (ber < best.ber) {
      best = { ber, offset: off, length: len };
      if (ber < ceiling) ceiling = ber;
    }
  }
  return best;
}
...
function matchChunkAgainstReferences(chunkFp, references, threshold = DEFAULT_BER_THRESHOLD) {
  ...
  for (const ref of references) {
    // Only a BER under the threshold, and under the best so far, can win:
    // let bestMatch abandon everything else early.
    const r = bestMatch(chunkFp, ref.fingerprint, {
      maxOffset: ref.fingerprint.length, maxBer: best ? best.ber : threshold,
    });
