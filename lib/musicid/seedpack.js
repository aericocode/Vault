 ...
 *           references (service's scanAgainstReferences onlyRefIds path).
...
async function _runRescan(onlyRefIds) {
  ...
  const only = new Set(onlyRefIds);
  for (const media_id of ids) {
    if (vault.isLocked()) throw new Error('interrupted (vault locked)');
    try {
      _job.rescan.new_links += service.scanAgainstReferences(media_id, { onlyRefIds: only }).length;
    } catch (e) {
