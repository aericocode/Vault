 ...
 * - Never the LAST copy of a model. Any copy may go while a sibling is still
 *   loaded; emptying the family from a panel labelled "load another copy" is a
 *   foot-gun, and the user's own LM Studio is the right place for that.
...
/**
 * The identifier for the next copy of `family`: the lowest free candidate from
 * `family`, `family:2`, `family:3`, ... upwards.
 *
 * The bare family id is a candidate like any other, and that is the fix for the
 * `…max:2:2` the user got: with the original ejected, `family` is free, so the
 * next copy takes it back and shows as `:1`. Numbering only ever comes off the
 * family, never off whatever suffixed id happened to be the source row. Gaps
 * are reused, so ejecting `:2` and loading again gives `:2` back rather than
 * creeping to `:4` over an afternoon of experimenting.
 */
function nextIdentifier(family, takenIds = []) {
  const used = new Set();
  const suffix = new RegExp(`^${family.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}:(\\d+)$`);
  for (const id of takenIds) {
    const text = String(id);
    if (text === family) { used.add(1); continue; }     // the original is loaded
...
function _finish(job, error) {
  ...
  job._poll = null;
  if (!error) _settleRegistry(job.identifier);
}

/**
 * Probe until the new copy is really in the registry, or give up.
 *
 * `lms ps` lists an identifier as soon as the load STARTS, so the job can call
 * itself done a second or two before LM Studio will admit the model is loaded
 * on /api/v0/models. One probe at that moment finds nothing and, now that
 * nothing else polls in the background, the copy would sit invisible until the
 * user pressed Refresh. So the probe repeats a few times, a couple of seconds
 * apart, and stops the moment the copy shows up.
 */
function _settleRegistry(identifier, tries = 6) {
  let slots;
  try { slots = require('./ai-slots'); } catch { return; }
  slots.refresh({ force: true }).then(reg => {
    const there = (reg?.slots || []).some(s => s.id === identifier);
    if (there || tries <= 1) return;
    const t = setTimeout(() => _settleRegistry(identifier, tries - 1), POLL_MS);
