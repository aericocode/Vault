 ...
 */
function _haltOnModel(item, reason) {
  ...
  if (!_halt) {
    _halt = { reason: String(reason || 'Model unavailable').slice(0, 300), filename: item?.filename || null, at: Date.now() };
  ...
  while (_queue.length > 0) {
    ...
    try {
      ...
      if (result?.modelUnavailable) {
        ...
        counted = false;
        requeued = _haltOnModel(item, result.error);
        break;
      }
      if (result?.error) {
        _failed++;
