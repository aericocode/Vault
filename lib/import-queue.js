...
_derive();
// An instance appearing or vanishing resizes the pool with no user action.
  ...
  while (_queue.length > 0) {
    ...
    try {
      ...
      if (result?.modelUnavailable) {
        ...
        counted = false;
        if (!result.needsModelChoice && _retryOnSurvivingInstance(item, result.error)) {
          requeued = true;
      ...
      if (vault.isLocked()) {
        ...
        if (!modelHealth.isModelChoiceNeeded(err) && _retryOnSurvivingInstance(item, reason)) {
          requeued = true;
        } else {
          requeued = _haltOnModel(item, reason,
