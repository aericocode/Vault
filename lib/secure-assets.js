 ...
 *
 * When vault mode is on (a VAULT_DB_PASSWORD is set and the encrypted
...
/**
 * Delete every asset of ONE kind for a media id. The HLS segment cache keeps
 * one row per segment (kind 'seg', lang = the segment number), so clearing a
 * file's cached segments must not take its thumbnail with it.
 * @returns {number} rows removed.
 */
function delKind(media_id, kind) {
  const db = conn();
  if (!db) return 0;
  return db.prepare('DELETE FROM assets WHERE media_id = ? AND kind = ?').run(media_id, kind).changes;
}

/** Total stored bytes for one kind of asset on one media id. */
function sizeOfKind(media_id, kind) {
  const db = conn();
  if (!db) return 0;
  const r = db.prepare('SELECT COALESCE(SUM(size),0) AS n FROM assets WHERE media_id = ? AND kind = ?')
    .get(media_id, kind);
  return r ? r.n : 0;
}
...
function migrateFromDisk() {
  ...
  // marker governs — no extra gate. But VAULT_SUBS can redirect it to an
...
module.exports = {
  ...
  get, has, put, del, delKind, sizeOfKind, deleteAll,
