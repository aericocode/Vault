 ...
 */
const DISCOVERY_MS = (() => {
  // A bare "AI_DISCOVERY_MS=" line in .env is unset, not zero: Number("") is 0
  // and would silently switch the sweep off.
  const text = String(process.env.AI_DISCOVERY_MS ?? "").trim();
  if (text === "") return 60_000;
